...
abstract class BaseHTTP
{
    ...
    protected function buildRequest($buf){
        ...
        $this->buildRequestArgs($httpRequestInfos->contentType, $requestBody, $args, $request);
        $request->setRequestMethod($httpRequestInfos->requestMethod);
        return $request;
    }

    private function buildRequestArgs($contentType, $requestBody, $args, Request $request){
        $requestBodyArr = $this->buildHttpRequestBody($contentType, $requestBody);
    ...
    private function buildHttpRequestBody($contentType, $requestBody){
        ...
        if(self::TYPE_X_WWW_FORM_URLENCODE == $contentType){
            parse_str(current($requestBody), $requestBodyArr);
        }
    ...
    private function buildHttpRequestSource($buf, &$body):RequestSource{
        ...
        $whenBody = false;
        while($httpOption = array_shift($httpOptions)){
            if($whenBody){
                break;
            }
            if(empty($httpOption)){
        ...
        $body = array_filter($httpOptions);
        $requestSource->contentLengthActual = 0;
        foreach($body as $bodyItem){
            $requestSource->contentLengthActual += strlen($bodyItem);
        }
        return $requestSource;
    }
